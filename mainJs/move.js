...
$(document).ready(function() {
    ...
    $('.btn1').css('background','#ff6600'); //첫번째 불켜
    $('.btn1').css('width','30px'); // 버튼의 너비 증가
    
    $('.gallery .link1').fadeIn('slow'); //첫번째 이미지 보인다..
    $('.gallery .link1 .spn1').delay(1000).animate({top:400, opacity:1},'slow');
    $('.gallery .link1 .spn2').delay(2000).animate({top:450, opacity:1},'slow');
    ...
    //   }
      ...
      $('.btn'+cnt).css('background','#ff6600');//자신만 불켜
      $('.btn'+cnt).css('width','30px');    

      $('.gallery ul li .spn1').css('top','300px').css('opacity','0'); //텍스트 모두 초기화
      $('.gallery ul li .spn2').css('top','350px').css('opacity','0');
      $('.gallery .link'+ cnt +' .spn1').delay(1000).animate({top:400, opacity:1},'slow'); //해당 텍스트 에니메이션
      $('.gallery .link'+ cnt +' .spn2').delay(2000).animate({top:450, opacity:1},'slow');
   ...
   $('.mbutton').click(function(event){  //각각의 버튼 클릭시
      ...
      $('.btn'+cnt).css('background','#ff6600');//자신 버튼만 불켜 
      $('.btn'+cnt).css('width','30px');
